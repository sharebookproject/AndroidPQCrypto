 ...
 * convert Shacal2 keys into a format usable by the CDC provider. The supported
 * KeySpec class is Shacal2KeySpec.
 * <p>
 ...
 */
public final class Shacal2KeyFactory extends SecretKeyFactory {

	/**
	 * Generate a Shacal2 key object from the provided key specification. The
	 * key specification has to be an instance of {@link de.flexiprovider.api.keys.SecretKeySpec} of type
	 * "Shacal2".
	 * 
	 * @param keySpec
	...
	public SecretKey generateSecret(KeySpec keySpec)
		...
		if (keySpec instanceof SecretKeySpec) {
			...
			if (secKeySpec.getAlgorithm().equals(Shacal2.ALG_NAME)) {
				return new Shacal2Key(secKeySpec.getEncoded());
			}
		}
	...
	public KeySpec getKeySpec(SecretKey key, Class keySpec)
		...
		if ((key == null) || !(key instanceof Shacal2Key)) {
			throw new InvalidKeySpecException("wrong key type");
		}

		return new SecretKeySpec(key.getEncoded(), "Shacal2");
	}

	/**
	 * Translates a Shacal2 key object, whose provider may be unknown or
