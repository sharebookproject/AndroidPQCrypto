 ...
 */
public class MeRSA extends AsymmetricBlockCipher {
     ...
     * <code>MeRSAPrivateKey</code>. Would be simpler to just check for
	...
	if (key instanceof MeRSAPrivateKey) {
	    return ((MeRSAPrivateKey) key).getN().bitLength();
	} else if (key instanceof RSAPublicKey) {
	    return ((RSAPublicKey) key).getN().bitLength();
	} else {
	    throw new InvalidKeyException("MeRSA: engineGetKeySize()"
	...
	if (!(key instanceof RSAPublicKey)) {
	    throw new InvalidKeyException("MeRSA: initEncrypt - "
	...
	if (params == null) {
	    ...
	    throw new InvalidParameterException("MeRSA: initEncrypt"
	...
	if (maxPlainTextSize <= 0) {
	    throw new InvalidParameterException(
		    "MeRSA: initEncrypt - Please use larger modulus!");
	}
    }
	...
	if (params == null) {
	    ...
	    throw new InvalidParameterException("MeRSA: initDecrypt"
		    + " - Invalid parameter specifaction!");
	}

	if (key instanceof MeRSAPrivateKey) {
	    privKey = (MeRSAPrivateKey) key;
	    pubKey = null;
	    ...
	    throw new InvalidKeyException("MeRSA: initDecrypt - "
		    + "RSASSA-OAEP needs a RSAPublicKey for encrypting data.");
	}

	if (maxPlainTextSize <= 0) {
	    throw new InvalidParameterException(
		    "MeRSA: initDecrypt - Please use larger modulus!");
	}
    }
	...
	// hash function (261 _ 1 octets for SHA-1), output _label too long_ and
	// stop.

	// b. If mLen > k _ 2hLen _ 2, output _message too long_ and stop.
	...
	try {
	    ...
	    // representative c
	    c = MeRSAOperations.MeRSAEP(pubKey, m);
	...
	// a. If the length of L is greater than the input limitation for the
	// hash function
	// (261 _ 1 octets for SHA-1), output _decryption error_ and stop.

	// b. If the length of the ciphertext C is not k octets, output
	// _decryption error_
	// and stop.
	if (input.length != cipherTextSize) {
	    throw new RuntimeException("MeRSA: messageDecrypt "
		    + "- decryption error");
	}

	// c. If k < 2hLen + 2, output _decryption error_ and stop.
	if (cipherTextSize < (md.getDigestLength() << 1) + 2) {
	    new RuntimeException("MeRSA: messageDecrypt "
		    + "- decryption error");
	}
	...
	try {
	    ...
	    throw new RuntimeException("MeRSA: PKCS1Exception - "
		    + ex.getMessage());
	}
