 ...
 * convert Twofish keys into a format usable by the CDC provider. The supported
 * KeySpec class is TwofishKeySpec.
 * <p>
 ...
 * @author Katja Rauch
 */
public class TwofishKeyFactory extends SecretKeyFactory {

	/**
	 * Generate a Twofish key object from the provided key specification. The
	...
	public SecretKey generateSecret(KeySpec keySpec)
		...
		if (keySpec instanceof SecretKeySpec) {
			...
			if (secKeySpec.getAlgorithm().equals(Twofish.ALG_NAME)) {
				return new TwofishKey(secKeySpec.getEncoded());
			}
		}
	...
	public KeySpec getKeySpec(SecretKey key, Class keySpec)
		...
		return new SecretKeySpec(key.getEncoded(), "Twofish");
	}

	/**
	 * Translates a Twofish key object, whose provider may be unknown or
