...
public class ElGamalKeyPairGenerator extends KeyPairGenerator {
     ...
     * the group <tt>(Z/pZ)*</tt> is generated. In the last step, a random
     * number <tt>a</tt> with <tt>0 < a < p-2</tt>, is chosen. The public
  ...
  /*  public KeyPair genKeyPair() {
	...
	do {
		...
		do{
		    	...
		    	do {
			    //germainPrime = new FlexiBigInt(keySize - 1, random);
			    strongPrime = new FlexiBigInt(keySize + 2, random);
			    //strongPrime = (germainPrime.shiftLeft(1))
				    //.add(FlexiBigInt.ONE);
			} while (strongPrime.bitLength() != keySize && !IntegerFunctions
				    .passesSmallPrimeTest(strongPrime)&&!strongPrime.isProbablePrime(CERTAINTY));
			do {
			    germainPrime = new FlexiBigInt(keySize - 1, random);
			  //  strongPrime = new FlexiBigInt(keySize + 2, random);
			    //strongPrime = (germainPrime.shiftLeft(1))
				    //.add(FlexiBigInt.ONE);
			} while (germainPrime.bitLength() != keySize && !IntegerFunctions
				    .passesSmallPrimeTest(germainPrime)&& !germainPrime.isProbablePrime(CERTAINTY));
