...
public final class ScalarMult {
	...
	di[0] = (y.multiply(new FlexiBigInt(Integer.toString(2))))
		.mod(prime); // d1

	// if(k >= 2) {
	C = di[0].multiply(di[0]).mod(prime);
	A = ((x.multiply(x).mod(prime).multiply(new FlexiBigInt(
		Integer.toString(3)))).add(a)).mod(prime);
	B = ((C.multiply(x)).multiply(new FlexiBigInt(Integer
	...
	T = (((x.multiply(x).mod(prime).multiply(new FlexiBigInt(
		Integer.toString(3)))).add(a)).multiply(li[0]))
		.mod(prime);
	...
	int wm = bm.bitLength();
	int Wm = wm + 1;
	int D = b1.mod(new FlexiBigInt(Integer.toString(1 << Wm)))
		.intValue();
	int d;
	while (D != 0 || i + wm < b) {
	    int x = D % (1 << Wm);
	...
	while (i > 0) {
		...
		indexr = new FlexiBigInt(Integer
			.toString((1 << (w + 1)) - 1));
